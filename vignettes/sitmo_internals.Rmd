...
`sitmo` is a Parallel Psuedo Random Number Generator (PPRNG). Support for `sitmo` exists for both C++ standards: C++98 and C++11. This package, however, only provides support for the C++11 use of SITMO due to the necessity to include `constexpr` to satisfy the clang on OS X. Furthermore, there are many different PPRNGs that are available: [trng](http://numbercrunch.de/trng/), [SPRNG](http://www.sprng.org/),  [RngStreams](http://statmath.wu.ac.at/software/RngStreams/doc/rngstreams.html),
